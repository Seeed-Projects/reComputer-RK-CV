...
# --- FastAPI 核心组件 ---
app = FastAPI(
    title="reComputer RK-CV YOLOv10 API",
    version="1.0.0",
    description="RK3588/RK3576 YOLOv10 object detection service",
)
...
@app.post("/api/models/yolov10/predict")
async def predict(
...
async def index():
    ...
    if is_web_only:
        ...
        <html>
          <head>
            <title>reComputer RK-CV YOLOv10 Web Analytics</title>
            <style>
          ...
          <body>
            <div class="container">
              <h1>YOLOv10 Offline Video Analytics</h1>
    ...
    <html>
      <head>
        <title>reComputer RK-CV Real-time YOLOv10 Detection</title>
        <style>
          body { background-color: #1a1a1a; color: white; text-align: center; font-family: sans-serif; margin: 0; padding: 20px; }
      ...
      <body>
        <div class="container">
          <h1>YOLOv10 Real-time Detection</h1>
...
def post_process_with_thresh(outputs, obj_thresh, nms_thresh):
    """Decode YOLOv10 DFL outputs and apply the official end-to-end Top-K."""
    if outputs is None:
        return None, None, None

    branch_count = 3
    if len(outputs) < branch_count * 2 or len(outputs) % branch_count != 0:
        raise ValueError(f"Unexpected YOLOv10 output count: {len(outputs)}")

    output_per_branch = len(outputs) // branch_count
    decoded_boxes = []
    class_scores = []
    for branch in range(branch_count):
        box_idx = output_per_branch * branch
        decoded_boxes.append(box_process(outputs[box_idx]))
        class_scores.append(outputs[box_idx + 1])
    ...
    scores = np.concatenate([sp_flatten(value) for value in class_scores])

    # Match rknn_model_zoo's two-stage Top-K without depending on PyTorch.
    max_det = min(300, boxes.shape[0])
    candidate_max_scores = np.max(scores, axis=-1)
    candidate_indices = np.argsort(candidate_max_scores)[-max_det:][::-1]
    boxes = boxes[candidate_indices]
    scores = scores[candidate_indices]

    flattened_scores = scores.reshape(-1)
    final_count = min(300, flattened_scores.size)
    final_indices = np.argsort(flattened_scores)[-final_count:][::-1]
    classes = final_indices % scores.shape[1]
    box_indices = final_indices // scores.shape[1]
    boxes = boxes[box_indices]
    scores = flattened_scores[final_indices]

    keep = scores > obj_thresh
    if not np.any(keep):
        return None, None, None

    # YOLOv10 is end-to-end/NMS-free; nms_thresh remains in the standard API
    # for compatibility with the other detector projects.
    _ = nms_thresh
    return boxes[keep], classes[keep].astype(np.int64), scores[keep]
...
def box_process(position):
    ...
    # Converted YOLOv10 models normally expose DFL regression tensors. Keep
    # support for already-decoded four-channel tensors for compatibility.
...
def main():
    parser = argparse.ArgumentParser(description='Web YOLOv10 detection on Rockchip NPU')
