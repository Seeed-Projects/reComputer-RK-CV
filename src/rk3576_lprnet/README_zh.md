...
本目录将 RKNN Model Zoo 的 LPRNet 案例封装为适用于 RK3576 的标准 reComputer RK-CV 服务，支持 RKNN NPU 识别、场景内车牌候选定位、浏览器实时预览、摄像头和本地视频输入、Web 上传视频分析、REST API 与 ARM64 Docker 镜像。
...
`PYTHONUNBUFFERED=1` 让日志立即输出；`RKNN_LOG_LEVEL=0` 隐藏已确认无害的 RKNN 静态模型初始化提示。`renderD129` 是 RK3576 测试设备上已验证的节点；若设备不同，请检查本机 `/dev/dri/`。
