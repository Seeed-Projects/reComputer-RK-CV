# RK3588 RetinaFace 部署指南

[English](./README.md) | [中文]

本目录将 RetinaFace 人脸检测封装为适用于 RK3588 的 reComputer RK-CV 标准服务，提供 RKNN NPU 推理、五点人脸关键点、浏览器预览、REST API、异步 MP4 处理和 ARM64 Docker 镜像。

## 核心特性

- 在 RK3588 NPU 上运行 320 × 320、MobileNet 骨干网络的 RetinaFace RKNN 模型。
...
访问 `http://<开发板IP>:8000` 上传图片并查看标注预览，或访问 `http://<开发板IP>:8000/docs` 查看 OpenAPI。若宿主机 8000 端口被占用，可改用 `-p 8001:8000`。
