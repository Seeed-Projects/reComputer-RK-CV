...
# 导入共享工具
from py_utils.resnet_utils import ResNet_helper
...
class VideoAnalyzer:
    def __init__(self, model=None, resnet_helper=None):
        self.model = model
        self.resnet_helper = resnet_helper
    ...
    def set_engine(self, model, resnet_helper):
        self.model = model
        self.resnet_helper = resnet_helper
    ...
    def _process_image(self, input_path, output_path):
        ...
        try:
            ...
            if self.model and self.resnet_helper:
                processed_img = self.resnet_helper.preprocess(frame)
                outputs = self.model.run(processed_img)

                if outputs is not None:
                    current_conf = det_config.get()
                    topk_classes, topk_scores = self.resnet_helper.get_topk(outputs, topk=5, conf_thresh=current_conf)
                    self.resnet_helper.draw_topk(frame, topk_classes, topk_scores)
...
# --- FastAPI 核心组件 ---
app = FastAPI(
    title="reComputer RK-CV ResNet50V2 API",
    version="1.0.0",
    description="RK3576 ResNet50V2 image and video classification service",
)
...
async def index():
    ...
    if is_web_only:
        ...
        <html>
          ...
          <body>
            <div class="container">
              <h1>ResNet50v2 Image & Video Analytics</h1>
    ...
    <html>
      ...
      <body>
        <div class="container">
          <h1>reComputer RK-CV Real-time ResNet50v2 Classification</h1>
          <div class="video-box">
...
_global_model = None
_global_resnet_helper = None

@app.post("/api/models/resnet50v2/predict")
async def predict(
    ...
    if _global_model is None or _global_resnet_helper is None:
    ...
    try:
        ...
        input_img = _global_resnet_helper.preprocess(img)
        ...
        topk_classes, topk_scores = _global_resnet_helper.get_topk(
...
def main():
    parser = argparse.ArgumentParser(description='Web ResNet50v2 classification on RK3576')
    parser.add_argument(
        '--model_path',
        type=str,
        default=str(BASE_DIR / 'model' / 'rk3576_resnet50-v2-7.rknn'),
    ...
    global _global_model, _global_resnet_helper

    # 初始化模型与辅助类
    model = RKNNLiteModel(args.model_path)
    resnet_helper = ResNet_helper(class_label_path=str(BASE_DIR / 'model' / 'synset.txt'))

    _global_model = model
    _global_resnet_helper = resnet_helper

    # 为视频分析器设置引擎
    video_analyzer.set_engine(model, resnet_helper)
    ...
    try:
        while True:
            ...
            # 推理流程
            processed_img = resnet_helper.preprocess(frame)
            ...
            if outputs is not None:
                current_conf = det_config.get()
                topk_classes, topk_scores = resnet_helper.get_topk(outputs, topk=5, conf_thresh=current_conf)
                resnet_helper.draw_topk(frame, topk_classes, topk_scores)
