# RK3576 DeepLabV3 部署指南

[English](./README.md) | [中文]

本目录提供面向 RK3576 的标准化 DeepLabV3 语义分割部署，包含 RKNN NPU 推理、浏览器预览、REST API、视频处理和 Docker 一键运行。

## 核心特性

- **NPU 加速**：通过 RKNN Toolkit Lite2 在 RK3576 上运行 `deeplabv3.rknn`。
...
更换模型时，将针对 RK3576 转换的 `deeplabv3.rknn` 放入 `model/`。模型应保持 21 类输出布局，否则需同步修改 `task_runtime.py` 中的 `LABELS` 和输出处理。
