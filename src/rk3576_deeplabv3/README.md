...
This directory provides a standardized DeepLabV3 semantic-segmentation deployment for RK3576, including RKNN NPU inference, browser preview, REST API, video processing, and Docker startup.

## Core Features

- **NPU acceleration**: Runs `deeplabv3.rknn` through RKNN Toolkit Lite2 on RK3576.
...
- `lib/`: RK3576 RKNN runtime library.
...
To replace the model, put a RK3576-converted `deeplabv3.rknn` in `model/`. Preserve the 21-class output layout or update `LABELS` and output handling in `task_runtime.py`.
