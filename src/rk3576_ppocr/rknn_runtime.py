...
class RKNNModel:
    """Thread-safe RKNNLite model wrapper."""
    ...
    def __init__(self, model_path, platform, core_index=None):
        ...
        if core_index == 0:
            ...
            core_mask = RKNNLite.NPU_CORE_0 if platform == "rk3576" else RKNNLite.NPU_CORE_2
        else:
            core_mask = RKNNLite.NPU_CORE_0_1 if platform == "rk3576" else RKNNLite.NPU_CORE_0_1_2
    ...
    def run(self, inputs, data_format=None):
        if not isinstance(inputs, (list, tuple)):
            inputs = [inputs]
        kwargs = {"inputs": [np.asarray(value) for value in inputs]}
