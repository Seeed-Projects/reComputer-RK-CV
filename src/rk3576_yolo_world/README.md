...
This directory provides a standardized YOLO-World deployment for RK3576: RKNN NPU inference, browser preview, REST API, and one-command Docker startup.

## Core Features

- **NPU acceleration**: Runs the YOLO-World RKNN model on the RK3576 NPU through RKNN Toolkit Lite2.
...
sudo docker run --rm --privileged --net=host \
  ...
  -v /proc/device-tree/compatible:/proc/device-tree/compatible:ro \
  ghcr.io/seeed-projects/recomputer-rk-cv/rk3576-yolo_world:latest \
  python web_detection.py --platform rk3576 --model_path model/yolo_world_v2s_i8.rknn \
