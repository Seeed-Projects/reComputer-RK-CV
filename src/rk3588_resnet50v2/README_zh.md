# RK3588 ResNet50V2 部署指南

[English](./README.md) | [中文]

本目录包含针对 RK3588 优化的 ResNet50V2 图像分类服务，提供基于 RKNN 的推理流程、浏览器预览、REST API，以及可直接运行的 Docker 镜像。

## 核心特性

- **硬件加速**：通过 RKNN-Toolkit-Lite2 在 RK3588 NPU 上运行 RKNN 模型。
...
- `model/`：RK3588 RKNN 模型、ImageNet 标签和示例图片。
- `py_utils/`：ResNet50V2 预处理与 Top-K 后处理工具。
- `video/`：容器默认命令使用的示例 MP4 视频。
- `web_classification.py`：提供 RKNN 推理、Web 预览和 FastAPI 接口的主程序。
- `requirements.txt`：Python 运行依赖。

默认模型为 `model/rk3588_resnet50-v2-7.rknn`，输入尺寸为 `224 x 224`。
...
- `web_classification.py` 负责加载 RKNN 模型、初始化 RK3588 NPU 运行时、处理摄像头或视频帧，并提供 Web/API 服务。
- `py_utils/resnet_utils.py` 将输入缩放至 `224 x 224`，把 BGR 输入转换为 RGB，执行 softmax 并筛选 Top-K 结果。
- 运行时上传文件和输出结果默认写入 `workspace/`，可通过 `RK_CV_WORKSPACE` 修改目录。

### 替换模型

1. 将兼容 RK3588 的 `.rknn` 模型放入 `model/`。
2. 确认模型输入布局、通道顺序、归一化方式和输出格式与 `ResNet_helper` 一致。
