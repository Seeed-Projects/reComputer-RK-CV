...
# --- FastAPI 核心组件 ---
app = FastAPI(
    title="reComputer RK-CV YOLOX API",
    version="1.0.0",
    description="RK3588/RK3576 YOLOX object detection service",
)
...
@app.post("/api/models/yolox/predict")
async def predict(
...
async def index():
    ...
    if is_web_only:
        ...
        <html>
          <head>
            <title>reComputer RK-CV YOLOX Web Analytics</title>
            <style>
          ...
          <body>
            <div class="container">
              <h1>YOLOX Offline Video Analytics</h1>
    ...
    <html>
      <head>
        <title>reComputer RK-CV Real-time YOLOX Detection</title>
        <style>
          body { background-color: #1a1a1a; color: white; text-align: center; font-family: sans-serif; margin: 0; padding: 20px; }
      ...
      <body>
        <div class="container">
          <h1>YOLOX Real-time Detection</h1>
...
def post_process_with_thresh(outputs, obj_thresh, nms_thresh):
    """Decode the three official YOLOX output branches and apply NMS."""
    if outputs is None:
        return None, None, None

    decoded_boxes, object_scores, class_scores = [], [], []
    for output in outputs:
        output = output.reshape(1, -1, output.shape[-2], output.shape[-1])
        decoded_boxes.append(box_process(output[:, :4]))
        object_scores.append(output[:, 4:5])
        class_scores.append(output[:, 5:])
    ...
    object_scores = np.concatenate([sp_flatten(value) for value in object_scores]).reshape(-1)
    class_scores = np.concatenate([sp_flatten(value) for value in class_scores])
    classes = np.argmax(class_scores, axis=-1)
    scores = class_scores[np.arange(class_scores.shape[0]), classes] * object_scores
...
def box_process(position):
    ...
    box_xy = (position[:, :2] + grid) * stride
    box_wh = np.exp(position[:, 2:4]) * stride
    xyxy = np.concatenate((box_xy - box_wh / 2, box_xy + box_wh / 2), axis=1)
    return xyxy
...
def main():
    parser = argparse.ArgumentParser(description='Web YOLOX detection on Rockchip NPU')
