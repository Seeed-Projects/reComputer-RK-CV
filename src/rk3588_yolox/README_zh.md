...
本目录包含针对 RK3588 优化的 YOLOX 目标检测服务，提供 RKNN 推理、浏览器预览、REST API、离线视频处理，以及可直接运行的 Docker 镜像。

## 核心特性

- **硬件加速**：通过 RKNN-Toolkit-Lite2 在 RK3588 NPU 上运行 YOLOX RKNN 模型。
- **COCO 目标检测**：使用 `640 x 640` letterbox 预处理，返回类别、置信度和检测框坐标。
- **多模型支持**：内置 YOLOX-S 和 YOLOX-M，默认使用 `yolox_s.rknn`。
- **模型专用后处理**：实现YOLOX 分支解码、目标置信度与类别置信度融合及按类别 NMS。
...
- `iou`：可选，IoU 阈值，接口默认值为 `0.45`。`iou` 字段用于控制本次请求的按类别 NMS。
...
全局置信度阈值用于预览、离线分析及未提供 `conf` 的推理请求。`iou` 字段用于控制本次请求的按类别 NMS。
...
- `web_detection.py` 负责初始化 RK3588 NPU 运行时、循环处理摄像头或视频输入、提供 FastAPI 接口，并实现YOLOX 分支解码、目标置信度与类别置信度融合及按类别 NMS。
- 输入画面经过 `640 x 640` letterbox 和 BGR 到 RGB 转换，检测框随后映射回原始画面尺寸。
- 运行时上传文件和输出结果默认写入 `workspace/`，可通过 `RK_CV_WORKSPACE` 修改目录。

### 替换模型

1. 将兼容 RK3588 的 YOLOX `.rknn` 模型放入 `model/`。
2. 确认模型输入尺寸和输出张量与 `web_detection.py` 中的预处理及 YOLOX 后处理一致。
