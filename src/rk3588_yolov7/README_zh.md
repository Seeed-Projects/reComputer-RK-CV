...
本目录包含针对 RK3588 优化的 YOLOv7 目标检测服务，提供 RKNN 推理、浏览器预览、REST API、离线视频处理，以及可直接运行的 Docker 镜像。

## 核心特性

- **硬件加速**：通过 RKNN-Toolkit-Lite2 在 RK3588 NPU 上运行 YOLOv7 RKNN 模型。
- **COCO 目标检测**：使用 `640 x 640` letterbox 预处理，返回类别、置信度和检测框坐标。
- **多模型支持**：内置 YOLOv7-Tiny 和完整 YOLOv7 模型，默认使用 YOLOv7-Tiny。
- **Anchor 解码**：加载 `anchors_yolov7.txt` 完成 YOLOv7 输出解码和 NMS。
- **灵活输入**：支持上传图片、提取 MP4 指定时间帧、本地 MP4 循环播放和摄像头输入。
- **Web 与 API**：提供浏览器预览、MJPEG 视频流、健康检查和 REST 推理接口。

## 目录结构

- `lib/`：RK3588 版 `librknnrt.so` 运行时库。
- `model/`：YOLOv7 RKNN 模型、Anchor 配置、COCO 标签和示例图片。
- `py_utils/`：letterbox、Anchor 加载、坐标还原、结果绘制和 RKNN 辅助工具。
- `video/`：容器默认命令使用的示例 MP4 视频。
- `web_detection.py`：提供目标检测、Web 预览和 FastAPI 接口的主程序。
- `requirements.txt`：Python 运行依赖。

可用模型包括 `model/yolov7-tiny.rknn` 和 `model/yolov7.rknn`。默认模型为 `model/yolov7-tiny.rknn`，Anchor 从 `model/anchors_yolov7.txt` 加载。

## 快速开始

### 1. 使用 Docker 运行

在 RK3588 开发板上直接运行已发布镜像：
...
- `web_detection.py` 负责初始化 RK3588 NPU 运行时、循环处理摄像头或视频输入、提供 FastAPI 接口，并实现 YOLOv7 Anchor 解码和 NMS。
- 输入画面经过 `640 x 640` letterbox 和 BGR 到 RGB 转换，检测框随后映射回原始画面尺寸。
- 运行时上传文件和输出结果默认写入 `workspace/`，可通过 `RK_CV_WORKSPACE` 修改目录。

### 替换模型

1. 将兼容 RK3588 的 YOLOv7 `.rknn` 模型放入 `model/`。
2. 确认模型输入尺寸和输出张量与 `web_detection.py` 中的 YOLOv7 预处理及后处理一致。
3. 准备匹配的 Anchor 文件，并通过 `--anchors model/<Anchor文件>.txt` 指定。
4. 如果模型不使用默认 COCO 80 类，请同步更新类别配置。
5. 使用 `--model_path model/<模型文件名>.rknn` 指定新模型。
