...
# 导入共享工具
from py_utils.coco_utils import COCO_test_helper, post_process, draw, CLASSES, load_anchors
...
# --- FastAPI 核心组件 ---
app = FastAPI(
    title="reComputer RK-CV YOLOv7 API",
    version="1.0.0",
    description="RK3588/RK3576 YOLOv7 object detection service",
)
...
@app.post("/api/models/yolov7/predict")
async def predict(
...
async def index():
    ...
    if is_web_only:
        ...
        <html>
          <head>
            <title>reComputer RK-CV YOLOv7 Web Analytics</title>
            <style>
          ...
          <body>
            <div class="container">
              <h1>YOLOv7 Offline Video Analytics</h1>
    ...
    <html>
      <head>
        <title>reComputer RK-CV Real-time YOLOv7 Detection</title>
        <style>
          body { background-color: #1a1a1a; color: white; text-align: center; font-family: sans-serif; margin: 0; padding: 20px; }
      ...
      <body>
        <div class="container">
          <h1>YOLOv7 Real-time Detection</h1>
...
def post_process_with_thresh(outputs, obj_thresh, nms_thresh):
    ...
    outputs = [_out.reshape([len(ANCHORS[0]), -1] + list(_out.shape[-2:])) for _out in outputs]
    for i in range(len(outputs)):
        boxes.append(box_process(outputs[i][:, :4, :, :], ANCHORS[i]))
        scores.append(outputs[i][:, 4:5, :, :])
        classes_conf.append(outputs[i][:, 5:, :, :])
...
def main():
    parser = argparse.ArgumentParser(description='Web YOLOv7 detection on Rockchip NPU')
    ...
    parser.add_argument('--anchors', type=str, default='model/anchors_yolov7.txt', help='YOLOv7 anchor file path')
