...
# --- FastAPI 核心组件 ---
app = FastAPI(
    title="reComputer RK-CV YOLOv6 API",
    version="1.0.0",
    description="RK3588/RK3576 YOLOv6 object detection service",
)
...
@app.post("/api/models/yolov6/predict")
async def predict(
...
async def index():
    ...
    if is_web_only:
        ...
        <html>
          <head>
            <title>reComputer RK-CV YOLOv6 Web Analytics</title>
            <style>
          ...
          <body>
            <div class="container">
              <h1>YOLOv6 Offline Video Analytics</h1>
    ...
    <html>
      <head>
        <title>reComputer RK-CV Real-time YOLOv6 Detection</title>
        <style>
          body { background-color: #1a1a1a; color: white; text-align: center; font-family: sans-serif; margin: 0; padding: 20px; }
      ...
      <body>
        <div class="container">
          <h1>YOLOv6 Real-time Detection</h1>
...
def post_process_with_thresh(outputs, obj_thresh, nms_thresh):
    ...
    default_branch = 3
    output_per_branch = len(outputs) // default_branch
    for i in range(default_branch):
        box_idx = output_per_branch * i
        score_idx = box_idx + 1
        boxes.append(box_process(outputs[box_idx]))
        classes_conf.append(outputs[score_idx])
        # YOLOv6 exports a third score-sum tensor for each branch. The official
        # rknn_model_zoo Python post-process intentionally ignores it; treating
        # it as objectness changes candidate ranking and produces invalid boxes.
        scores.append(np.ones_like(outputs[score_idx][:, :1, :, :], dtype=np.float32))
...
def box_process(position):
    ...
    # 官方 yolov6 既支持 DFL 回归输出，也支持已经回归为 4 通道的 ltrb 输出。
    # 当前 rk3588 模型的 box tensor 形状为 [1, 4, H, W]，如果错误地走 DFL，
    # 因为 mc=1，softmax 后会退化成全 0，最终得到 x1==x2、y1==y2 的“点框”。
...
def main():
    parser = argparse.ArgumentParser(description='Web YOLOv6 detection on Rockchip NPU')
